...
### Step 7a — Generate Conversation Transcript

Generate a human-readable Markdown transcript and save it as `transcript.md` in the job folder. Then tell the user it has been saved.
...
**Start date/time:** <The time and date the conversation was started>
**End date/time:** <The time and date when the template was finally rendered>
**Output:** <comma-separated output file basenames>

---
```

**Body** — one `##` section per group, in the same order as the interview (Phase 3):

- If `groups` is present in the manifest: iterate `manifest.groups` in order.
- If `groups` is absent: auto-derive in this order: (1) unconditional variables auto-grouped by affinity, (2) one section per entry in `manifest.conditionals`, (3) one section per entry in `manifest.loops`.
...
**Conditional group — included** (condition was true):
```
## <Group Name> *(included)*
...
Only add `*(included)*` / `*(skipped)*` / `*(alternative)*` annotations when `manifest.conditionals` is non-empty — suppress entirely for simple templates with no conditionals.

**Conditional group — skipped** (condition false, no else-branch):
```
## <Group Name> *(skipped — not applicable)*
```
Heading only, no table.

**Conditional group — else-branch taken:**
```
...
...
```
If the collection is empty: `*(no items recorded)*`.
...
#### Determining conditional evaluation results

Reconstruct from the variable dictionary:
- `gate_type: boolean` → condition was true if `variable_dict[condition_var]` is `True`
- `gate_type: equality` → condition was true if `variable_dict[gate_variable] == gate_value`
- Gate variable absent from dict → treat as false (skipped)

#### Label and value formatting

- Labels: use `variable.label` from manifest; fallback: title-case the variable name (`bank_name` → `Bank Name`)
- `True` → `Yes`, `False` → `No`; empty/`None` → `—`; all other types: display as-is
...
If writing fails, log the error and continue — document delivery always takes priority.
