...
description: Complete guide to writing CoQuill templates — placeholders, conditionals, loops, and rules
---

CoQuill templates are standard `.docx`, `.html`, or `.md` files with Jinja2-style markup embedded in the content. This guide covers everything you need to write templates that work correctly with the interview engine.
...
In `.docx` files, place placeholders directly in the document text. CoQuill uses `docxtpl`, which preserves all formatting around the placeholder. In `.html` files, placeholders can appear anywhere in the body, attributes, or `<style>` blocks.
...
- **One template file per directory** — each template directory contains exactly one `.docx`, `.html`, or `.md` file, not mixed
...
| Feature | `.docx` | `.html` | `.md` |
|---------|---------|---------|-------|
| Engine | `docxtpl` | `jinja2` | `jinja2` |
| Output | `.docx` + optional `.pdf` | `.html` + `.pdf` | `.md` + optional `.pdf` |
| Formatting | Preserved from template | Controlled by CSS | Standard Markdown rendering |
| Syntax | Same Jinja2 syntax | Same Jinja2 syntax | Same Jinja2 syntax |
| Tables | Native Word tables | HTML `<table>` | Markdown tables |
| Best for | Legal docs, formatted contracts | Invoices, web-ready documents | Meeting notes, internal records |

For legal documents where formatting matters (line spacing, fonts, headers), `.docx` is usually the right choice. For documents that will be sent digitally or embedded in web workflows, `.html` + PDF is more flexible. For lightweight documents like meeting notes, action item lists, or internal records, `.md` is the simplest option — it's easy to author, version-control friendly, and can optionally produce a PDF.
