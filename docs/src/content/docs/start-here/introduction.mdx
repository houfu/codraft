---
title: Introduction
description: What CoQuill is, the problem it solves, and how it works
---

CoQuill is a document assembly tool built on [Claude Cowork](https://claude.com/product/cowork). It is similar to traditional document automation platforms — [docassemble](https://docassemble.org/) (which this project takes a lot of inspiration from), HotDocs, Documate — but approaches the process using AI first. The result is a tool that is much simpler: a conversational interview driven by Claude, producing completed documents from plain templates. See [How CoQuill Compares](/coquill/feature-tour/how-it-compares/) for a detailed breakdown.
...
CoQuill works for any document that follows a consistent structure with variable content:
...
CoQuill is not a general document editor. It is not a CMS, a form builder, or a word processor. It is a fill-in-the-blanks engine: the template author controls the structure, the AI collects the values, and the output is a completed instance of that template.

## Ready to start?

→ [Quickstart](/coquill/start-here/quickstart/)
